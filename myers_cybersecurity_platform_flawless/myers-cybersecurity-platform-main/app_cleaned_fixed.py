...
# config import removed; use os.environ directly
import os
import secrets
import logging
import pandas as pd
...
if 'first_name' in st.session_state and 'last_name' in st.session_state and 'email' in st.session_state:'
    st.write(f"Welcome, {st.session_state.first_name} {st.session_state.last_name} ({st.session_state.email})!")"
...
def show_subscription_page():"
    ...
    if details:"
        ...
        if details.get('is_trial'):'
            st.write(f"**Trial Ends:** {details.get('trial_ends').strftime('%Y-%m-%d') if details.get('trial_ends') else 'N/A'}")"
        ...
        billing_period = details.get('billing_period', 'monthly') # Assuming this field exists
...
def show_api_keys_page():"
    ...
    api_keys = security_core.list_api_keys(id)
    if api_keys:
        keys_data = []
        for key in api_keys:
            keys_data.append({"
                ...
                if selected_key_info['status'] == 'active':'
                    if col_action.button("Deactivate", key="deactivate_api_key_button", help="Deactivate this API key"):"
                        with st.spinner(f"Deactivating key {selected_key_info['name']}..."):
                            if security_core.deactivate_api_key(selected_key_id, id):"
                    ...
                    if col_action.button("Activate", key="activate_api_key_button", type="primary", help="Activate this API key"):"
                        with st.spinner(f"Activating key {selected_key_info['name']}..."):
                            if security_core.activate_api_key(selected_key_id, id):"
                    ...
                    if st.warning(f"Are you sure you want to permanently delete API Key '{selected_key_info['name']}'? This cannot be undone."):"
                        if st.button("Confirm Delete", key="confirm_delete_api_key", type="secondary"):"
                            with st.spinner(f"Deleting key {selected_key_info['name']}..."):
                                if security_core.delete_api_key(selected_key_id, id):"
            ...
            if not key_name or not service:"
                ...
                with st.spinner("Generating new API key..."):
                    generated_key_id = security_core.add_api_key(
                        id, key_name, new_api_key_value, service, permissions
                    )
                    if generated_key_id:"
...
def forgot_password_page(security_core_instance: from security_core_pg import SecurityCore, email_automation_instance: from email_automation import EmailAutomation):"
    ...
    if st.button("Send Reset Link", key="send_reset_link_button", type="primary"):
        if email:"
            with st.spinner("Sending password reset link..."):
                try:
                    # In from security_core_pg import SecurityCore, generate_password_reset_token should handle user lookup and token storage
                    token = security_core_instance.generate_password_reset_token(email) 
                    if token:"
                        app_base_url = os.environ['APP_URL', 'http://localhost:8501')'
...
def reset_password_page(security_core_instance: from security_core_pg import SecurityCore):"
    ...
    if st.button("Reset Password", key="reset_password_button", type="primary"):
        if new_password and confirm_password:
            if new_password == confirm_password:
                # Validate password strength using the from security_core_pg import SecurityCore method
                ...
                if not is_strong:
                    ...
                    with st.spinner("Resetting your password..."):
                        try:
                            # In from security_core_pg import SecurityCore, reset_password should hash the password and update DB"
                            if security_core_instance.reset_password(info['id'], new_password):'
        ...
        if submitted:
            if email and password:"
                with st.spinner("Logging in..."):
                    auth_result, error_message = st.session_state.security_core.authenticate_user(email, password)
