...
class SetupWizard:
    ...
    def api_key_setup_step(self):'
        ...
        st.markdown("Set up your first API keys and configure automated security scanning.")"
    ...
    def final_review_step(self):'
        ...
        st.markdown("Please review your configuration before completing the setup.")
        ...
        with col2:"
            ...
            if setup_data['api_keys_data']:'
                ...
                st.write("No API keys configured (can be added later)")
    ...
    def complete_setup(self):'
        ...
        try:
            # Create admin user
            user_idid, msg = self.security_core.create_user("
            ...
            if user_idid:
                ...
                cursor = conn.cursor()'
                cursor.execute('UPDATE users SET role = %s WHERE id = %s', ('admin', user_idid))
                conn.commit()
                conn.close()
                ...
                st.session_state.user_idid = user_idid"
                st.session_state.user_idemail = setup_data['admin_email']'
                st.session_state.user_idrole = 'admin''
