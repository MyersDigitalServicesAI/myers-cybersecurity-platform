...
def show_admin_panel(security_core):
    ...
    user_idrole = st.session_state.get('user_idrole')'
    if user_idrole != 'admin':'
        st.error("Access Denied: You must be an administrator to view this page.")
        return"
    st.subheader("Admin Actions")"
    st.write("Here you can manage users, view system logs, and configure global settings.")
    ...
    try:
        ...
        if users_data:
            ...
            user_idto_change_email = st.text_input("Enter email of user to change role:")
            if user_idto_change_email:
                target_user = security_core.get_user_idby_email(user_idto_change_email)
                if target_user:"
