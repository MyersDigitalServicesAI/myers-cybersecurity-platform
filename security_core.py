...
class SecurityCore:
    def __init__(self):"
        self.SECRET_KEY = os.environ["JWT_SECRET_KEY"]  # Load from env"
    ...
    def init_database(self):"
        ...
        try:
            ...
            cursor.execute('''CREATE TABLE IF NOT EXISTS api_keys (
                id VARCHAR(255) PRIMARY KEY,
                user_idid VARCHAR(255) REFERENCES users(id) ON DELETE CASCADE,
...
return None
    ...
    @log_api_call
    def promote_to_admin(self, user_idid: str) -> bool:"
        ...
        try:
            ...
            if cursor.rowcount == 0:'
                logger.warning(f"User {user_idid} not found for promotion.")
                return False"
            logger.info(f"User {user_idid} promoted to admin.")
            return True
        except Exception as e:"
            logger.error(f"Error promoting user {user_idid} to admin: {e}", exc_info=True)
            if conn:
                conn.rollback()
            raise
        finally:
    ...
    @log_api_call
    def demote_to_user(self, user_idid: str) -> bool:"
        ...
        try:
            ...
            conn.commit()
            if cursor.rowcount == 0:'
                logger.warning(f"User {user_idid} not found for demotion.")
                return False"
            logger.info(f"User {user_idid} demoted to user.")
            return True
        except Exception as e:"
            logger.error(f"Error demoting user {user_idid} to user: {e}", exc_info=True)
            if conn:
                conn.rollback()
            raise
        finally:
    ...
    @log_api_call
    def deactivate_user_idaccount(self, user_idid: str) -> bool:"
        ...
        try:
            ...
            conn.commit()
            if cursor.rowcount == 0:'
                logger.warning(f"User {user_idid} not found for deactivation.")
                return False"
            self.log_security_event(user_idid, "user_idaccount_deactivated", "info", f"User account {user_idid} deactivated.")"
            logger.info(f"User account {user_idid} deactivated.")
            return True
        except Exception as e:"
            logger.error(f"Error deactivating user account {user_idid}: {e}", exc_info=True)
            if conn:
                conn.rollback()
            raise
        finally:
    ...
    @log_api_call
    def activate_user_idaccount(self, user_idid: str) -> bool:"
        ...
        try:
            ...
            conn.commit()
            if cursor.rowcount == 0:'
                logger.warning(f"User {user_idid} not found for activation.")
                return False"
            self.log_security_event(user_idid, "user_idaccount_activated", "info", f"User account {user_idid} activated.")"
            logger.info(f"User account {user_idid} activated.")
            return True
        except Exception as e:"
            logger.error(f"Error activating user account {user_idid}: {e}", exc_info=True)
            if conn:
                conn.rollback()
            raise
        finally:
    ...
    def create_user(self, email: str, password: str, company: str, first_name: str, last_name: str, plan: str,"
        ...
        if self.get_user_idby_email(validated_email):"
        ...
        user_idid = secrets.token_urlsafe(16)
        ...
        try:
            ...
            ''', (user_idid, validated_email, password_hash, company, first_name, last_name,
            ...
            conn.commit()'
            logger.info(f"User {user_idid} created successfully.")"
            return user_idid, "User created successfully."
    ...
    def get_user_idby_email(self, email: str) -> Optional[Dict[str, Any]]:"
    ...
    def get_user_iddetails(self, user_idid: str) -> Optional[Dict[str, Any]]:"
        ...
        try:
            ...
            logger.error(f"Error retrieving user details for {user_idid}: {e}", exc_info=True)
            raise # Re-raise for caller to handle
        finally:
            if conn:
                return_db_connection(conn)

    def get_user_idid_by_email(self, email: str) -> Optional[str]:"
    ...
    def get_user_idid_by_subscription_id(self, subscription_id: str) -> Optional[str]:"
    ...
    def verify_user_idemail(self, user_idid: str) -> bool:"
        ...
        try:
            ...
            conn.commit()
            if cursor.rowcount == 0:'
                logger.warning(f"User {user_idid} not found for email verification.")
                return False"
            logger.info(f"Email verified for user {user_idid}.")
            return True
        except Exception as e:"
            logger.error(f"Error verifying user email {user_idid}: {e}", exc_info=True)
            if conn:
                conn.rollback()
            return False
        finally:
    ...
    def authenticate_user(self, email: str, password: str) -> Tuple[Optional[Dict[str, Any]], Optional[str]]:"
        ...
        """
        user = self.get_user_idby_email(email)
        if not user:"
    ...
    def update_user_idsubscription_status(self, user_idid: str, payment_status: str, is_trial: Optional[bool] = None,
        ...
        Args:
            user_idid (str): The ID of the user to update.'
        ...
        if payment_status not in PAYMENT_STATUSES:"
            logger.error(f"Invalid payment status '{payment_status}' provided for user {user_idid}.")
            return False
        
        if plan is not None and plan not in ALLOWED_PLANS:"
            logger.error(f"Invalid plan '{plan}' provided for user {user_idid}.")
            return False
        ...
        try:
            ...
            params.append(user_idid)

            cursor.execute(update_sql, tuple(params))
            conn.commit()
            if cursor.rowcount == 0:"
                logger.warning(f"User {user_idid} not found for subscription status update.")
                return False"
            logger.info(f"User {user_idid} subscription status updated to {payment_status}. is_trial: {is_trial}, sub_id: {subscription_id}, plan: {plan}")
            return True
        except Exception as e:"
            logger.error(f"Error updating user {user_idid} subscription status: {e}", exc_info=True)
            if conn:
                conn.rollback()
            raise
        finally:
    ...
    def get_trial_discount_eligibility(self, user_idid: str) -> bool:"
        """Checks if a user is eligible for a trial conversion discount (within first 15 days of trial)."""
        conn = None
        try:
            conn = get_db_connection()
            cursor = conn.cursor()"
            cursor.execute('SELECT trial_start_date, is_trial FROM users WHERE id = %s', (user_idid,))
            result = cursor.fetchone()
            ...
            logger.error(f"Error checking trial discount eligibility for user {user_idid}: {e}", exc_info=True)
            raise
        finally:
            if conn:
                return_db_connection(conn)"
    def calculate_discounted_price(self, base_price: float, plan_name: str, billing_period: str = "monthly", user_idid: Optional[str] = None) -> Dict[str, Any]:"
        """
        ...
        Args:
            ...
            user_idid (Optional[str], optional): The ID of the user for specific discounts. Defaults to None.
        ...
        user_idinfo = None
        if user_idid:
            user_idinfo = self.get_user_iddetails(user_idid) # Fetch user info to check auto_renewal"
        ...
        if user_idinfo and user_idinfo.get('auto_renewal', False) and billing_period == "monthly":
        ...
        if billing_period == "yearly":
            ...
            if user_idid and self.get_trial_discount_eligibility(user_idid):
    ...
    @log_api_call'
    def add_api_key(self, user_idid: str, name: str, api_key: str, service: str, permissions: str = "read") -> Optional[str]:"
        """
        Adds an encrypted API key for a user.

        Args:
            user_idid (str): The ID of the user.
        ...
        try:
            ...
            cursor = conn.cursor()"
            cursor.execute('''
                INSERT INTO api_keys (id, user_idid, name, encrypted_key, service, permissions, last_used, status)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s)'
            ''', (key_id, user_idid, name, encrypted_key, service, permissions, datetime.now(), 'active'))
            conn.commit()'
            self.log_security_event(user_idid, "api_key_created", "info", f"API key '{name}' created for service '{service}'")"
            logger.info(f"API key '{name}' added for user {user_idid}.")
            return key_id
        except Exception as e:"
            logger.error(f"Error adding API key for user {user_idid}: {e}", exc_info=True)
            if conn:
                conn.rollback()
    ...
    @log_api_call
    def get_api_key(self, key_id: str, user_idid: str) -> Optional[Dict[str, Any]]:"
        ...
        try:
            ...
            cursor.execute("""
                ...
                FROM api_keys WHERE id = %s AND user_idid = %s"
            """, (key_id, user_idid))
            result = cursor.fetchone()
            if result:
                return {"
            ...
            logger.error(f"Error retrieving API key {key_id} for user {user_idid}: {e}", exc_info=True)
            raise
        finally:
            if conn:
                return_db_connection(conn)
    ...
    @log_api_call
    def list_api_keys(self, user_idid: str) -> List[Dict[str, Any]]:"
        ...
        try:
            ...
            cursor.execute("""
                SELECT id, name, encrypted_key, service, permissions, created_at, last_used, status
                FROM api_keys WHERE user_idid = %s"
            """, (user_idid,))
            ...
            logger.error(f"Error listing API keys for user {user_idid}: {e}", exc_info=True)
            raise
        finally:
            if conn:
                return_db_connection(conn)
    ...
    @log_api_call
    def delete_api_key(self, key_id: str, user_idid: str) -> bool:"
        ...
        try:
            ...
            cursor = conn.cursor()"
            cursor.execute("""
                DELETE FROM api_keys WHERE id = %s AND user_idid = %s"
            """, (key_id, user_idid))
            conn.commit()
            if cursor.rowcount == 0:"
                logger.warning(f"API key {key_id} not found or does not belong to user {user_idid} for deletion.")
                return False"
            self.log_security_event(user_idid, "api_key_deleted", "info", f"API key {key_id} deleted.")"
            logger.info(f"API key {key_id} deleted for user {user_idid}.")
            return True
        except Exception as e:"
            logger.error(f"Error deleting API key {key_id} for user {user_idid}: {e}", exc_info=True)
            if conn:
                conn.rollback()
            raise
        finally:
    ...
    @log_api_call
    def deactivate_api_key(self, key_id: str, user_idid: str) -> bool:"
        ...
        try:
            ...
            cursor = conn.cursor()"
            cursor.execute("""
                UPDATE api_keys SET status = %s, last_used = %s WHERE id = %s AND user_idid = %s"
            """, ('inactive', datetime.now(), key_id, user_idid))
            conn.commit()
            if cursor.rowcount == 0:'
                logger.warning(f"API key {key_id} not found or does not belong to user {user_idid} for deactivation.")
                return False"
            self.log_security_event(user_idid, "api_key_deactivated", "info", f"API key {key_id} deactivated.")"
            logger.info(f"API key {key_id} deactivated for user {user_idid}.")
            return True
        except Exception as e:"
            logger.error(f"Error deactivating API key {key_id} for user {user_idid}: {e}", exc_info=True)
            if conn:
                conn.rollback()
            raise
        finally:
    ...
    @log_api_call
    def activate_api_key(self, key_id: str, user_idid: str) -> bool:"
        ...
        try:
            ...
            cursor = conn.cursor()"
            cursor.execute("""
                UPDATE api_keys SET status = %s WHERE id = %s AND user_idid = %s"
            """, ('active', key_id, user_idid))
            conn.commit()
            if cursor.rowcount == 0:'
                logger.warning(f"API key {key_id} not found or does not belong to user {user_idid} for activation.")
                return False"
            self.log_security_event(user_idid, "api_key_activated", "info", f"API key {key_id} activated.")"
            logger.info(f"API key {key_id} activated for user {user_idid}.")
            return True
        except Exception as e:"
            logger.error(f"Error activating API key {key_id} for user {user_idid}: {e}", exc_info=True)
            if conn:
                conn.rollback()
            raise
        finally:
    ...
    @log_api_call
    def log_security_event(self, user_idid: str, event_type: str, severity: str, description: str, source_ip: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None) -> bool:"
        """
        Logs a security event to the database.

        Args:
            user_idid (str): The ID of the user associated with the event."
        ...
        try:
            ...
            cursor = conn.cursor()"
            cursor.execute('''
                INSERT INTO security_events (id, user_idid, event_type, severity, description, source_ip, metadata)
                VALUES (%s, %s, %s, %s, %s, %s, %s)'
            ''', (event_id, user_idid, event_type, severity, description, source_ip, json.dumps(metadata) if metadata else None))
            conn.commit()'
            logger.debug(f"Security event '{event_type}' logged for user {user_idid}.")
            return True
        except Exception as e:"
            logger.error(f"Error logging security event for user {user_idid}: {e}", exc_info=True)
            if conn:
                conn.rollback()
            return False
        finally:
    ...
    @log_api_call
    def get_security_events(self, user_idid: str, limit: int = 100, offset: int = 0) -> List[Dict[str, Any]]:"
        ...
        try:
            ...
            cursor.execute("""
                ...
                FROM security_events WHERE user_idid = %s
                ORDER BY timestamp DESC LIMIT %s OFFSET %s"
            """, (user_idid, limit, offset))
            results = cursor.fetchall()
            events = []
            for result in results:
                events.append({"
            ...
            logger.error(f"Error retrieving security events for user {user_idid}: {e}", exc_info=True)
            raise
        finally:
            if conn:
                return_db_connection(conn)
...
if __name__ == '__main__':
    ...
    test_email = f"test_user_id{secrets.token_hex(4)}@example.com""
    test_password = "SecurePassword123!"
    user_idid, msg = security_core.create_user(
        email=test_email,
    ...
    if user_idid:"
        print(f"User created: {user_idid}, message: {msg}")"
        ...
        print(f"\nVerifying email for user {user_idid}...")
        if security_core.verify_user_idemail(user_idid):"
            ...
            if authenticated_user:"
                ...
                print(f"Authentication failed: {auth_error}")"
            print(f"\nPromoting user {user_idid} to admin...")
            if security_core.promote_to_admin(user_idid):"
                print(f"User {user_idid} is now an admin.")
            else:"
                print("Failed to promote user.")"
            print(f"\nDeactivating user {user_idid} account...")
            if security_core.deactivate_user_idaccount(user_idid):"
                print(f"User {user_idid} account deactivated.")
            else:"
            ...
            if authenticated_user:"
                ...
                print(f"Authentication failed: {auth_error}")"
            print(f"\nActivating user {user_idid} account...")
            if security_core.activate_user_idaccount(user_idid):"
                print(f"User {user_idid} account activated.")
            else:"
    ...
    if user_idid:"
        ...
        key_id = security_core.add_api_key(user_idid, "My Test Key", api_key_value, "TestService", permissions="write")
        if key_id:"
            print(f"API Key added with ID: {key_id}")
            encrypted_key_data = security_core.get_api_key(key_id, user_idid)
            if encrypted_key_data:"
                ...
                if security_core.deactivate_api_key(key_id, user_idid):"
                    print(f"API key {key_id} deactivated.")
                    # Verify status
                    key_info = security_core.get_api_key(key_id, user_idid)"
                    print(f"API key status after deactivation: {key_info['status']}")"
                    print(f"\nActivating API key {key_id}...")
                    if security_core.activate_api_key(key_id, user_idid):"
                ...
                if security_core.delete_api_key(key_id, user_idid):"
    ...
    monthly_price_new_user = security_core.calculate_discounted_price(100.0, "basic", "monthly", user_idid=user_idid)
    ...
    security_core.update_user_idsubscription_status(user_idid, "active", is_trial=False, auto_renewal=True)"
    yearly_price_existing_user = security_core.calculate_discounted_price(100.0, "professional", "yearly", user_idid=user_idid)
    print(json.dumps(yearly_price_existing_user, indent=2))"
    print("\nMonthly Plan for Existing User (auto-renewal OFF, not trial eligible)")"
    security_core.update_user_idsubscription_status(user_idid, "active", is_trial=False, auto_renewal=False)"
    monthly_price_existing_user_idno_renewal = security_core.calculate_discounted_price(100.0, "basic", "monthly", user_idid=user_idid)
    print(json.dumps(monthly_price_existing_user_idno_renewal, indent=2))

    # Clean up user and key for re-runs
    if user_idid:
        try:"
            print(f"\nCleaning up test user {user_idid}...")"
            # Deactivate to ensure it's in a known state before deletion (if you had hard delete policy)
            security_core.deactivate_user_idaccount(user_idid)
            ...
            cursor_cleanup.execute("DELETE FROM users WHERE id = %s", (user_idid,))
            conn_cleanup.commit()
            return_db_connection(conn_cleanup)"
            print(f"Test user {user_idid} and associated data deleted.")
        except Exception as e:"
